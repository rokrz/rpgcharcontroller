...
const SUPPLEMENT = fs.existsSync(SUPPLEMENT_PATH) ? JSON.parse(fs.readFileSync(SUPPLEMENT_PATH, "utf8")) : {
  class: {},
  subclass: {}
};
const RACE_SUPPLEMENT_PATH = path.join(__dirname, "../data/dnd5e_race_supplement.json");
const RACE_SUPPLEMENT = fs.existsSync(RACE_SUPPLEMENT_PATH) ? JSON.parse(fs.readFileSync(RACE_SUPPLEMENT_PATH, "utf8")) : {};
const BG_SUPPLEMENT_PATH = path.join(__dirname, "../data/dnd5e_backgrounds_supplement.json");
const BG_SUPPLEMENT = fs.existsSync(BG_SUPPLEMENT_PATH) ? JSON.parse(fs.readFileSync(BG_SUPPLEMENT_PATH, "utf8")) : {};
const FEATS_PATH = path.join(__dirname, "../data/dnd5e_feats.json");
const ALL_FEATS = fs.existsSync(FEATS_PATH) ? JSON.parse(fs.readFileSync(FEATS_PATH, "utf8")) : [];
const SPELL_LEVELS_PATH = path.join(__dirname, "../data/dnd5e_spell_levels.json");
const SPELL_LEVELS = fs.existsSync(SPELL_LEVELS_PATH) ? JSON.parse(fs.readFileSync(SPELL_LEVELS_PATH, "utf8")) : {};
...
function getSupplementClassFeatures(slug, maxLevel) {
  ...
  return entries.filter(f => f.level <= maxLevel).map(supplementToFeature);
}
...
function getSrdClassFeaturesByClass(className, maxLevel) {
  ...
  return Object.values(srd).filter(f => f.class && f.class.toLowerCase() === className.toLowerCase() && !f.subclass && f.level <= maxLevel).map(f => ({
    ...f,
    type: "features"
  }));
}
function getSrdSubclassFeaturesByName(subclassName, maxLevel) {
  const srd = getSrdFeatures();
  return Object.values(srd).filter(f => f.subclass && f.subclass.toLowerCase() === subclassName.toLowerCase() && f.level <= maxLevel).map(f => ({
    ...f,
    type: "features"
  }));
}
...
async function searchDnd5e(type, query, page = 1, limit = 20) {
  ...
  if (!query || allItems.length === 0) {
    const paged = paginate(allItems.map(item => ({
      index: item.index,
      name: item.name,
      url: item.url
    })), page, limit);
  ...
  const {
    ...
    total,
    totalPages
  } = paginate(allItems, page, limit);
  const detailed = await Promise.allSettled(pageItems.slice(0, limit).map(item => fetch(`https://www.dnd5eapi.co${item.url}`).then(r => r.json())));
  const results = detailed.filter(r => r.status === "fulfilled").map(r => normalizeResult(r.value, type));
  return {
    results,
    total,
    page: Number(page),
...
function normalizeResult(raw, type) {
  ...
  if (type === "spells") {
    var _raw$school, _raw$components, _raw$desc, _raw$higher_level;
    return {
      ...base,
      school: (_raw$school = raw.school) === null || _raw$school === void 0 ? void 0 : _raw$school.name,
      level: raw.level,
      castingTime: raw.casting_time,
      range: raw.range,
      components: (_raw$components = raw.components) === null || _raw$components === void 0 ? void 0 : _raw$components.join(", "),
      duration: raw.duration,
      concentration: raw.concentration,
      ritual: raw.ritual,
      description: (_raw$desc = raw.desc) === null || _raw$desc === void 0 ? void 0 : _raw$desc.join("\n\n"),
      higherLevel: (_raw$higher_level = raw.higher_level) === null || _raw$higher_level === void 0 ? void 0 : _raw$higher_level.join("\n\n")
    };
  }
  if (type === "equipment") {
    var _raw$equipment_catego, _raw$damage$damage_ty, _raw$desc2, _raw$properties;
    return {
      ...base,
      category: (_raw$equipment_catego = raw.equipment_category) === null || _raw$equipment_catego === void 0 ? void 0 : _raw$equipment_catego.name,
      cost: raw.cost ? `${raw.cost.quantity} ${raw.cost.unit}` : undefined,
      weight: raw.weight,
      damage: raw.damage ? `${raw.damage.damage_dice} ${(_raw$damage$damage_ty = raw.damage.damage_type) === null || _raw$damage$damage_ty === void 0 ? void 0 : _raw$damage$damage_ty.name}` : undefined,
      armorClass: raw.armor_class,
      description: (_raw$desc2 = raw.desc) === null || _raw$desc2 === void 0 ? void 0 : _raw$desc2.join("\n\n"),
      properties: (_raw$properties = raw.properties) === null || _raw$properties === void 0 ? void 0 : _raw$properties.map(p => p.name).join(", ")
    };
  }
  if (type === "features") {
    var _raw$class, _raw$subclass, _raw$desc3;
    return {
      ...base,
      class: (_raw$class = raw.class) === null || _raw$class === void 0 ? void 0 : _raw$class.name,
      subclass: (_raw$subclass = raw.subclass) === null || _raw$subclass === void 0 ? void 0 : _raw$subclass.name,
      level: raw.level,
      description: (_raw$desc3 = raw.desc) === null || _raw$desc3 === void 0 ? void 0 : _raw$desc3.join("\n\n")
    };
  }
  if (type === "monsters") {
    var _raw$armor_class, _raw$special_abilitie;
    return {
      ...base,
      type: raw.type,
      cr: raw.challenge_rating,
      hp: raw.hit_points,
      ac: (_raw$armor_class = raw.armor_class) === null || _raw$armor_class === void 0 || (_raw$armor_class = _raw$armor_class[0]) === null || _raw$armor_class === void 0 ? void 0 : _raw$armor_class.value,
      description: (_raw$special_abilitie = raw.special_abilities) === null || _raw$special_abilitie === void 0 ? void 0 : _raw$special_abilitie.map(a => `**${a.name}**: ${a.desc}`).join("\n\n")
    };
  }
  return {
    ...base,
    description: ((_raw$desc4 = raw.desc) === null || _raw$desc4 === void 0 ? void 0 : _raw$desc4.join("\n\n")) || raw.description
  };
}
...
async function getClassFeatures(slug, maxLevel = 20) {
  ...
  const merged = [...supplementFeatures, ...srdFeatures.filter(f => !supplementNames.has(f.name.toLowerCase()))];
  if (merged.length > 0) {
  ...
  // 3. API fallback
  const levelResults = await Promise.all(Array.from({
    length: maxLevel
  }, (_, i) => fetch(`${DND5E_BASE}/classes/${slug}/levels/${i + 1}/features`).then(r => r.ok ? r.json() : {
    results: []
  }).catch(() => ({
    results: []
  }))));
  const refs = levelResults.flatMap((r, i) => (r.results || []).map(f => ({
    ...f,
    level: i + 1
  })));
  const details = await Promise.allSettled(refs.slice(0, 30).map(f => fetch(`https://www.dnd5eapi.co${f.url}`).then(r => r.json()).then(raw => ({
    ...normalizeResult(raw, "features"),
    level: f.level
  }))));
  const results = details.filter(r => r.status === "fulfilled").map(r => r.value);
...
async function getSubclassFeatures(subclassSlug, maxLevel = 20) {
  ...
  const details = await Promise.allSettled(refs.slice(0, 20).map(f => fetch(`https://www.dnd5eapi.co${f.url}`).then(r => r.json()).then(raw => normalizeResult(raw, "features"))));
  const results = details.filter(r => r.status === "fulfilled").map(r => r.value).filter(f => !maxLevel || (f.level || 0) <= maxLevel);
...
function normalizeTraitDetails(raw, isSubrace) {
  ...
  return {
    ...
    abilityBonuses: (raw.ability_bonuses || []).map(b => {
      var _b$ability_score, _b$ability_score2;
      return {
        abilityName: ABILITY_NAME_MAP[(_b$ability_score = b.ability_score) === null || _b$ability_score === void 0 ? void 0 : _b$ability_score.index] || ((_b$ability_score2 = b.ability_score) === null || _b$ability_score2 === void 0 ? void 0 : _b$ability_score2.name),
        bonus: b.bonus
      };
    }),
    skillProficiencies: profs.filter(p => p.index.startsWith("skill-")).map(p => {
      const name = (p.name || "").replace("Skill: ", "");
      return {
        key: apiSkillToKey(p.index),
        name: SKILL_NAME_PT[name] || name
      };
    }),
    otherProficiencies: profs.filter(p => !p.index.startsWith("skill-")).map(p => p.name),
...
function mergeRaceDetails(parent, sub) {
  var _sub$speed, _sub$darkvision;
  ...
  return {
    index: sub.index,
    name: sub.name,
    speed: (_sub$speed = sub.speed) !== null && _sub$speed !== void 0 ? _sub$speed : parent.speed,
    darkvision: (_sub$darkvision = sub.darkvision) !== null && _sub$darkvision !== void 0 ? _sub$darkvision : parent.darkvision,
...
async function getRaceDetails(raceIndex) {
  ...
  if (isSubrace) {
    ...
    const parentIndex = (_subRaw$race = subRaw.race) === null || _subRaw$race === void 0 ? void 0 : _subRaw$race.index;
...
async function getClassDetails(slug) {
  ...
  if (!res.ok) return null;
  const raw = await res.json();
  const proficiencyChoices = (raw.proficiency_choices || []).map(choice => {
    var _choice$from;
    const options = (((_choice$from = choice.from) === null || _choice$from === void 0 ? void 0 : _choice$from.options) || []).filter(o => {
      var _o$item;
      return (_o$item = o.item) === null || _o$item === void 0 || (_o$item = _o$item.index) === null || _o$item === void 0 ? void 0 : _o$item.startsWith("skill-");
    }).map(o => ({
      key: apiSkillToKey(o.item.index),
  ...
  const savingThrows = (raw.saving_throws || []).map(st => st.index.replace("saving-throw-", ""));
  const result = {
    index: slug,
    name: raw.name,
    proficiencyChoices,
...
async function getBackgroundDetails(bgIndex) {
  ...
  const result = {
    ...
    otherProficiencies: profs.filter(p => !p.index.startsWith("skill-")).map(p => p.name),
    languageOptions: ((_raw$language_options = raw.language_options) === null || _raw$language_options === void 0 ? void 0 : _raw$language_options.choose) || 0,
    feature: raw.feature ? {
      name: raw.feature.name,
      desc: (raw.feature.desc || []).join("\n")
    } : null
  };
...
async function searchClassSpells(classSlug, query, maxSpellLevel, page = 1, limit = 20) {
  ...
  const details = await Promise.allSettled(pageRefs.map(r => fetch(`https://www.dnd5eapi.co${r.url}`).then(res => res.json()).then(raw => normalizeResult(raw, "spells"))));
...
async function searchClassSpellsUnion(classSlugs, query, maxSpellLevel, page = 1, limit = 20) {
  const allRefsArrays = await Promise.all(classSlugs.map(async classSlug => {
    const listKey = `spells-${classSlug}`;
  ...
  merged.sort((a, b) => {
    var _SPELL_LEVELS$a$index, _SPELL_LEVELS$b$index;
    const la = (_SPELL_LEVELS$a$index = SPELL_LEVELS[a.index]) !== null && _SPELL_LEVELS$a$index !== void 0 ? _SPELL_LEVELS$a$index : 99;
    const lb = (_SPELL_LEVELS$b$index = SPELL_LEVELS[b.index]) !== null && _SPELL_LEVELS$b$index !== void 0 ? _SPELL_LEVELS$b$index : 99;
...
function searchFeats(query) {
  const q = (query || "").toLowerCase();
  const filtered = q ? ALL_FEATS.filter(f => f.name.toLowerCase().includes(q) || (f.description || "").toLowerCase().includes(q)) : ALL_FEATS;
