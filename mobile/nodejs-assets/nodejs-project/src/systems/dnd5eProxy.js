...
async function searchDnd5e(type, query) {
  ...
  const detailed = await Promise.allSettled(items.slice(0, 10).map(item => fetch(`https://www.dnd5eapi.co${item.url}`).then(r => r.json())));
  return detailed.filter(r => r.status === "fulfilled").map(r => normalizeResult(r.value, type));
}
...
async function getRaceDetails(raceIndex) {
  ...
  const endpoint = isSubrace ? `${DND5E_BASE}/subraces/${raceIndex}` : `${DND5E_BASE}/races/${raceIndex}`;
...
async function searchClassSpells(classSlug, query, maxSpellLevel) {
  ...
  const details = await Promise.allSettled(top.map(r => fetch(`https://www.dnd5eapi.co${r.url}`).then(res => res.json()).then(raw => normalizeResult(raw, "spells"))));
  let spells = details.filter(r => r.status === "fulfilled").map(r => r.value);
  if (maxSpellLevel > 0) spells = spells.filter(s => {
    var _s$level;
    return ((_s$level = s.level) !== null && _s$level !== void 0 ? _s$level : 0) <= maxSpellLevel;
  });
  return spells;
}
