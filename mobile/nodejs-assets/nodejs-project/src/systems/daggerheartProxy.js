...
async function searchCards(query) {
  ...
  try {
    ...
    const items = ((_json$data = json.data) === null || _json$data === void 0 ? void 0 : _json$data.classFeatures) || [];
...
const SAMPLE_CARDS = [{
  ...
  level: 1,
  type: "Foundation",
  description: "You gain +1 to attack rolls when outnumbered.",
  mechanics: "+1 attack when outnumbered."
}, {
  index: "second-wind",
  name: "Second Wind",
  class: "Warrior",
  level: 1,
  type: "Foundation",
  description: "Once per short rest, heal 1d6 + proficiency HP as a bonus action.",
  mechanics: "1d6+Prof HP, once per short rest."
}, {
  ...
  mechanics: "+2d6 damage with advantage."
}];
async function getClassCards(className) {
  ...
  try {
    ...
    const json = await res.json();
    const items = ((_json$data2 = json.data) === null || _json$data2 === void 0 ? void 0 : _json$data2.classFeatures) || [];
