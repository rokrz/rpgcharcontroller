...
const { searchDnd5e, getClassFeatures, getSubclasses, getSubclassFeatures, getRaceDetails, getBackgroundDetails, getClassDetails, searchClassSpells, searchClassSpellsUnion, getRaceList, searchFeats, paginate } = require("../systems/dnd5eProxy");
...
router.get("/dnd5e", async (req, res) => {
  const { type = "spells", search = "", class: cls = "", level = "20", index = "", maxlevel = "0", page = "1", limit = "20" } = req.query;
  ...
  try {
    ...
    if (type === "classspellsunion") {
      ...
      if (!slugs.length) return res.json({ results: [], total: 0, totalPages: 0, page: pg });
