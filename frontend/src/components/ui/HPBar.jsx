...
export default function HPBar({ current, max, temp = 0 }) {
  ...
  const tempPct = safeMax > 0 ? Math.min((safeTemp / safeMax) * 100, 100 - pct) : 0;
  ...
  return (
    <div className="flex items-center gap-2 min-w-0">
      ...
      <div className="flex-1 h-2 bg-parchment border border-parchment-edge rounded-sheet overflow-hidden min-w-[40px] flex">
        <div className={`h-full ${FILL_CLS[status]} transition-all`} style={{ width: `${pct}%` }} />
        {safeTemp > 0 && (
          <div className="h-full bg-gold transition-all" style={{ width: `${tempPct}%` }} />
        )}
      </div>
      <span className={`tabular-nums text-xs shrink-0 ${TEXT_CLS[status]}`}>
        {safeCurrent}{safeTemp > 0 ? `+${safeTemp}` : ""}/{safeMax}
      </span>
    </div>
  );
}
